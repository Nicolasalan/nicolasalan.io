...
Para criar uma incporporacao de token ao patches que possa ser aprendida, precisa criar em uma formado,
`embedding_dimension` chamado de (*D*) e adicionar a uma dimensao `number_of_patches` chamado de (*N*).
...
<Alert type="attention">
  A concatenacao acontece `torch.cat()`com dimensao `dim=1`.
</Alert>

Para criar uma incporporacao de token ao patches que possa ser aprendida, precisa do `batch_size` e dimensao
do patches, depois cria um `torch.ones()` (tensor de 1) com o tamanho do `batch_size` e `embedding_dimension`, e depois passa
para o `torch.nn.Parameter()` para que possa ser aprendido.
...
<Alert type="attention">
  Aqui esta criando com `torch.ones`, mas na realidade utiliza o `torch.randn()`, para gerar dados aleatorios,
